...
export abstract class BasePage {
    ...
    protected async fillText1(element: Locator, textToFill: string) {
        await test.step(`Filling '${textToFill}' into the '${element}' element`, async() => {
            await element.fill(textToFill);
        })
    }

    protected async fillText(element: string, textToFill: string) {
        await test.step(`Filling '${textToFill}' into the '${element}' element`, async() => {
            await this.page.locator(element).fill(textToFill);
        })
    }
}